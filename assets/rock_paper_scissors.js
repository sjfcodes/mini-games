const playGame = (data) => {
    ...
    else if (uChoice === 'r' && cChoice === 's' || uChoice === 'p' && cChoice === 'r' || uChoice === 's' && cChoice === 'p') handleResult('win')
    else handleResult('lose')
    return !(alert(`${JSON.stringify(data, null, 4)}`)) && confirm('play again?') && playGame(data)
}
document.querySelector('#r-p-s').addEventListener('click', () => !(alert('welcome to rock, paper, scissors!')) && playGame({ rounds: 0, win: 0, lose: 0, tie: 0, options: ['r', 'p', 's'] }));